...
function initMap(position, pointOfInterest, events, view, zoom){
  ...
  addEvents(layerEventGroup, events,'visitor');
...
function addPositionMarkerOnMap(map, layerPositionGroup, position, zoom){
  ...
  let marker = L.marker([position.lat, position.lon],{icon:customIcon('red'),autoPan:true}).addTo(layerPositionGroup);
...
function addressObjectToString(addressObject){
  ...
  function setPointOfInterestMarker(data, address, layerPointOfInterestGroup){
    ...
    if(data.type === "node"){
        ...
        if(data.tags.amenity === 'restaurant'){
          ...
          marker.bindPopup("<h4>"+data.tags.name+"<h4><h1>"+data.tags.amenity+"</h1><p><b>Adresse : </b>"+address+"</p>")
        }
          ...
          //marker.bindPopup("<h4>"+data.tags.name+"<h4><h1>"+data.tags.tourism+"</h1><p><b>Adresse : </b>"+address+"</p>")
        }
  ...
  */
  function addEvents(layerEventGroup, events, view){
    console.log(events)
    // On parcourt les données et on ajoute un marqueur pour chaque évènement de data
    if(events != undefined){
      for(let data of events){
        setEventMarker(data,layerEventGroup,view);
      }
    }
    if(view == 'visitor'){
      console.log("oui")
        setEventMarker(setFakeEventMarkerPosition(userPosition), layerEventGroup,view)
      }
    }
    ...
    // Ajout d'évènement mouserover aux markers
      if(view == 'user'){
        ...
        marker.bindPopup('<h4>Veuillez vous inscrire pour afficher les '+
                         ...
                         '<p><a href='+"xd"+'>Formulaire d\'inscription</a><p>'
                         ,{
          maxWidth: '400',
          width: '200',
  ...
  */
  function getButton(bar,buttonName){
    ...
    function setFakeEventMarkerPosition(position){
      var obj = position;
