...
function addPositionMarkerOnMap(map, layerPositionGroup, position, zoom){
  ...
  let marker = L.marker([position.lat, position.lon],{icon:customIcon('red'),autoPan:false,draggable:true}).addTo(layerPositionGroup);
  ...
  function setEventMarker(data, layerEventGroup){
    ...
    marker.on('click', function (e) {
      ...
      if(sidebar.isVisible())
        (lastEventOpened == data.id) ? (sidebar.update(), sidebar.toggle() )
                                     : sidebar.update(data)
      else{sidebar.update(data);sidebar.toggle();}
    });
  }
  ...
  function addButtonsOnMap(bar){
      ...
      states:[{
        ...
        onClick: function(button){
          CenterMapOnUserPosition(userPosition,13);
        },
