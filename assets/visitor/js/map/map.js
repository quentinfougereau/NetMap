...
function initMap(position, pointOfInterest, events, view, zoom){
  ...
  if(view === 'user') {
    ...
    // Controleur de la Popup des évènements
    controlInit(mymap)
  }
  ...
  //eventControlInit(mymap); // Controler gérant l'affichage des popups d'évènements
...
function addPositionMarkerOnMap(map, layerPositionGroup, position, zoom){
  ...
  (view == 'user') ? marker = L.marker([position.lat, position.lon],{
                                      icon:customIcon('red'),autoPan:false,draggable:true}
                                  ).addTo(layerPositionGroup) :
                     marker = L.marker([position.lat, position.lon],{
                                      icon:customIcon('red'),autoPan:false,draggable:false}
                                  ).addTo(layerPositionGroup)

  getDataAddress(position,'user',marker,addToolTipOnMarker);

  // Centre la carte selon la position du marqueur
  CenterMapOnUserPosition(position,map.getZoom());
  ...
  if(view == 'user'){
    ...
    marker.on('dragend', function(event) {
      ...
      CenterMapOnUserPosition(userPosition,map.getZoom());
    });
  }
}
 ...
 */
 function addCircleOnPosition(layerPositionGroup,position,color,fillColor,opacity,fillOpacity,radius){
   ...
   layerPositionGroup.clearLayers();
   if(radius > 800)
     mymap.setView([userPosition.lat, userPosition.lon], mymap.getZoom());
   else{
     mymap.setView([userPosition.lat, userPosition.lon], mymap.getZoom());
    }
    ...
    if(data.address.hasOwnProperty(property))
      if(data.address[property] != undefined)
        switch (property){
          ...
          case 'road':addressObject.road = data.address[property];break;
        }
...
function addressObjectToString(addressObject){
  ...
  */
  function setPointOfInterestMarker(data, address, layerPointOfInterestGroup){
    ...
    if(data.type === "node" && data.tags.name != undefined && address.length > 2){
        ...
        if(data.tags.amenity === 'restaurant'){
          ...
          // Suppression des espaces par des + pour la recherche google
          let link = 'https://www.google.com/search?q='+data.tags.name+"+"+address
          link = link.replace(/ /g,'+')
          link = link.substring(0, link.length-1);
          console.log(link);
          marker.bindPopup('<h1>'+data.tags.name+'</h1>'+
                           '<p><b>Type : </b>'+data.tags.amenity+'<p>'+
                           '<p><b>Adresse : </b>'+address+'</p>'+
                           '<p><b>Cliquez <a href="'+link+'"target="_blank">ici</a> pour obtenir plus d\'informations sur ce lieu.</b></p>'
                           ,{
            maxWidth: '400',
            width: '200',
            className : 'POIPopup'
          ...
          link = link.substring(0, link.length-1);
          marker.bindPopup('<h1>'+data.tags.name+'</h1>'+
                           '<p><b>Type : </b>'+data.tags.tourism+'<p>'+
                           '<p><b>Adresse : </b>'+address+'</p>'+
                           '<p><b>Cliquez <a href="'+link+'">ici</a> pour obtenir plus d\'informations sur ce lieu.</b></p>'
                           ,{
            maxWidth: '400',
            width: '200',
            className : 'POIPopup'
        ...
        marker.on('click', function (e) {
          ...
          marker.getPopup().isOpen() ? marker.openPopup() : marker.closePopup();
        });
    }
  }
  ...
  */
  function addEvents(layerEventGroup, events){
    ...
    setFakeEventMarkerPosition(userPosition,layerEventGroup);
  }
  ...
  */
  function setEventMarker(data, layerEventGroup,view){
    var marker = L.marker(
      [data.lat, data.lon],{
      icon:L.divIcon({
    ...
    // Ajout d'évènement mouserover aux markers
      if(view == 'user'){
        ...
        var img = '<i class="fas fa-lock fa-lg lockImg"></i>'
        marker.bindPopup('<h4>Veuillez vous inscrire pour afficher les '+
                         'évènements autours de votre position.</h4>'+img+
                         '<p><a href="../views/register.php">Formulaire d\'inscription</a><p>'
                         ,{
          maxWidth: '400',
          width: '200',
  ...
  */
  ...
  // Créé un faux marqueur pour appâter l'utilisateur
  // La desipcription de la fonction ressemble beaucoup trop
  // à celle d'une carte Yu-Gi-Oh.
  function setFakeEventMarkerPosition(position,layerEventGroup){
    var obj = position;
    obj.lat = +obj.lat + 0.005;
    obj.lon = +obj.lon
